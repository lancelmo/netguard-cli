...
O projeto foi construído utilizando o concept de ambiente virtual isolado (`venv`) e depende das seguintes bibliotecas de terceiros:
...
No Arch Linux / Manjaro: ```bash
sudo pacman -S libpcap --noconfirm

No Ubuntu / Debian: ```bash
...
sniffer.py: Módulo de monitoring contínuo (SnifferModule). Captura o tráfego IP de forma promíscua, identificando fabricantes via endereço MAC e classificando a segurança dos produtos em tempo real (RF03/RF04).
